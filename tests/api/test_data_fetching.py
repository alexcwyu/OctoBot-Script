...
async def test_social_historical_data():
    with mock.patch.object(backtesting_api, "initialize_and_run_data_collector", mock.AsyncMock(return_value="social_data")) \
         as initialize_and_run_data_collector_mock:
        assert await obs.social_historical_data(
...
async def test_get_data_with_social_services(mocked_config):
    with mock.patch.object(data_fetching, "historical_data",
                           mock.AsyncMock(return_value="exchange_data")) as historical_data_mock, \
            mock.patch.object(data_fetching, "social_historical_data",
                              mock.AsyncMock(return_value="social_data")) as social_historical_data_mock, \
            mock.patch.object(backtesting_api, "create_and_init_backtest_data",
                              mock.AsyncMock(return_value="backtest_data")) as create_and_init_backtest_data_mock:
        assert await obs.get_data(
            "BTC/USDT",
...
async def test_get_data_with_auto_social_services(mocked_config):
    with mock.patch.object(data_fetching, "historical_data",
            ...
            mock.patch.object(data_fetching, "social_historical_data",
                              mock.AsyncMock(return_value="social_data")) as social_historical_data_mock, \
            mock.patch.object(data_fetching.octobot_mocks, "get_activated_social_services",
                              mock.Mock(return_value=["AlternativeMeServiceFeed"])) as get_activated_social_services_mock, \
            mock.patch.object(backtesting_api, "create_and_init_backtest_data",
                              mock.AsyncMock(return_value="backtest_data")) as create_and_init_backtest_data_mock:
        assert await obs.get_data(
            "BTC/USDT",
...
async def test_get_data_with_tentacles_config(mocked_config):
    ...
    with mock.patch.object(data_fetching, "historical_data",
                           mock.AsyncMock(return_value="exchange_data")) as historical_data_mock, \
            mock.patch.object(data_fetching.octobot_mocks, "get_activated_social_services",
                              mock.Mock(return_value=[])) as get_activated_social_services_mock, \
            mock.patch.object(data_fetching.octobot_mocks, "get_tentacles_config",
                              mock.Mock(return_value=TEST_TENTACLES_CONFIG)) as get_tentacles_config_mock, \
            mock.patch.object(backtesting_api, "create_and_init_backtest_data",
                              mock.AsyncMock(return_value="backtest_data")) as create_and_init_backtest_data_mock:
        assert await obs.get_data(
            "BTC/USDT",
